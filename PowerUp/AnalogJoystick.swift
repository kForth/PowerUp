...
public struct AnalogJoystickData: CustomStringConvertible {
    ...
    mutating func reset(preserve_vel vel: Bool = false, preserve_angle angle: Bool = false) {
        if !vel {
...
open class AnalogJoystick: SKNode {
    ...
    open override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first, substrate.calculateAccumulatedFrame().contains(touch.location(in: self)) {
//            stick.position = touch.location(in: self)
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch: AnyObject in touches {
            ...
            let velocity = CGPoint(x: needPosition.x / maxDistantion, y: needPosition.y / maxDistantion)
            data = AnalogJoystickData(velocity: velocity, angular: -atan2(needPosition.x, needPosition.y))
        }
    ...
    private func resetStick() {
        ...
        data.reset(preserve_angle: true)
        stopHandler?();
    }
    
    private var touch_begin_callbacks = [() -> ()]()
    private var touch_moved_callbacks = [() -> ()]()
    private var touch_ended_callbacks = [() -> ()]()
